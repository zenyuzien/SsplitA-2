...
intersection.nI = 0 ;
```
'intersection' would be the name to be used for below explanations..<br>
...
// furthur processing with intersection list 
```

There is a problem with this however. Every packet is doing this hence, multiple duplicate intersections would occur. That is why, duplicates from the intersection list is done.
```cpp
...
```
Now the intersections are ready. 

### 5. SSA-2
if you recap the structures required to implement SSA-2 :
```cpp
struct ssa_utility 
{
    int nf ;
...
```
the purpose of table is to directly access clauses with specific rule numbers. The idea is that the intersection set is a vector. The order of which you push back clauses will remain same so whenever 6th intersection {1,4,5} occurs, table[0],table[4],table[5] will append '5' because 6th intersection means index 5 (0-index based). So When the program needs all clauses of rule number x, it doesn't have to linear search entire intersection list CL, but simply iterate through table[x-1] to get all clauses of x <br>
That was the idea but due to two types of clauses - positive and negative, for every intersection, two clauses are inserted to CL. Eg. if {1,4,5} is an intersection, CL will be appended 2 vectors - {1,4,5} and {-1,-4,-5}
